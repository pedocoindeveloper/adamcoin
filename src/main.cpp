...
uint256 hashGenesisBlock("0xc217486b35cbd6a5b8fed819440f6674a9dd4947dbbc52bf3f906ee2f8a3e6a0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // adamcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "adamcoin Signed Message:\n";
...
{
    // adamcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 1) //bounties, bonuses, get the ADAMcoin going
        { 
                nSubsidy = 50000 * COIN;
    ...
    // Subsidy is cut in half every 696969 blocks, which will occur approximately every 556.6 days
    nSubsidy >>= (nHeight / 696969); // adamcoin: 696969 blocks in 556.6 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // adamcoin: 60 minutes
static const int64 nTargetSpacing = 60; // adamcoin: 60 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // adamcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // adamcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // adamcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Pedocoin launch Adam Guerbuez";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710faa89ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1497193843;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 1978420;
        ...
        assert(block.hashMerkleRoot == uint256("0x130369b14363c97df84a11ca152a69bc5d42279062d6ed38e74a15f2f08c9ef2"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // adamcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("adamcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("adamcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static adamcoinMiner(CWallet *pwallet)
{
    printf("adamcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("adamcoin-miner");
    ...
    try { loop {
        ...
        printf("Running adamcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("adamcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&adamcoinMiner, pwallet));
}
